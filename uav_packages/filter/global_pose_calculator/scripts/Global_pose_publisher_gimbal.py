...
class MyNode:
    def __init__(self,Name = "Position_calculation"): #TrajX,Trajy,
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer([self.Target_relative_pose, self.UAV_odom, self.gimbal_orientation], queue_size=10, slop=0.1, allow_headerless=True)
    ...
    def callback(self, PoseStamped_msg,odom_msg, gimb_orien_msg):
        ...
        print("\n\nbefore correction: \nx=",X_pos,"\ny=",Y_pos,"\nz=",Z_pos)
        for i in range(len(gimb_orien_msg.name)):
            if gimb_orien_msg.name[i]=="uav1::servo_camera_link":
                x=gimb_orien_msg.pose[i].orientation.x
                y=gimb_orien_msg.pose[i].orientation.y
                z=gimb_orien_msg.pose[i].orientation.z
                w=gimb_orien_msg.pose[i].orientation.w
        ...
        [[X_pos],[Y_pos],[Z_pos]] = np.dot(rotation_matrix,[[X_pos],[Y_pos],[Z_pos]])
        ...
        if PoseStamped_msg.pose.position.z >500 : 
            ...
            print("before publishing correction: \nX = ",X_pos,"\nY = ",Y_pos)
