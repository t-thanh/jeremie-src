...
class Constraint_Verification{
  ...
  Eigen::VectorXd rho_a_plus;
  Eigen::VectorXd K_plus;
  Eigen::VectorXd rho_v_plus;
  bool constraint_ok;
    ...
    if(counter == 0){
      ...
      for(int i = 0;  i < sum.size();i++){
        if(sum(i) > 0) { 
          ...
          K_plus(new_index-1) = Psn_constraint(old_index-1)*factor*K_(old_index-1) +K_(old_index-1);
          rho_v_plus(new_index-1) = Vel_constraint(old_index-1)*factor*rho_v_(old_index-1)+ rho_v_(old_index-1);
          rho_a_plus(new_index-1) = Acc_constraint(old_index-1)*factor*rho_a_(old_index-1)+ rho_a_(old_index-1);
          K_plus(new_index) = Psn_constraint(old_index-1)*factor *K_(old_index-1) + K_(old_index-1);
          rho_v_plus(new_index) = Vel_constraint(old_index-1)*factor*rho_v_(old_index-1)+rho_v_(old_index-1);
          rho_a_plus(new_index) = Acc_constraint(old_index-1)*factor*rho_a_(old_index-1)+rho_a_(old_index-1);

          }
          else{

		            
          K_plus(i) = Psn_constraint(i)*factor*K_(old_index-1)+K_(old_index-1);
          rho_v_plus(i) = Vel_constraint(i)*factor*rho_v_(old_index-1)+rho_v_(old_index-1);
          rho_a_plus(i) = Acc_constraint(i)*factor*rho_a_(old_index-1)+rho_a_(old_index-1);
          K_plus(i+1) = Psn_constraint(i)*factor*K_(old_index-1)+K_(old_index-1);
          rho_v_plus(i+1) = Vel_constraint(i)*factor*rho_v_(old_index-1)+rho_v_(old_index-1);
          rho_a_plus(i+1) = Acc_constraint(i)*factor*rho_a_(old_index-1)+rho_a_(old_index-1);
          ...
          K_plus(new_index-1) = K_(old_index-1);
          rho_v_plus(new_index-1) = rho_v_(old_index-1);
          rho_a_plus(new_index-1) = rho_a_(old_index-1);
          }
