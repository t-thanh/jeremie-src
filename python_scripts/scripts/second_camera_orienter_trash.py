...
class GimbalMover:
    def __init__(self):
        ...
        self.current_roll=0
        self.corr_quat=np.array([0, -0.7071, 0, 0.7071])
    ...
    def correct_target_position(self, gimbal_orientation, target_position):
        ...
        print("Uncorrected gimbal quaternion: ",quaternion)
        
        corrected_orient = quaternion_multiply(quaternion,self.corr_quat)
    ...
    def callback(self, target_msg, gimb_orient_msg):
        ...
        if self.data[0]>limit: #anti wind-up
            ...
            self.data[0]=-limit
        if self.data[1]>=1.5708+limit:
            self.data[1]=1.5708+limit
        elif self.data[1]<1.5708-limit:
            self.data[1]=1.5708-limit
        
        self.angle_msg.data = self.data
        #self.angle_msg.data = [0.05,1.5708-0.05]
