...
class GimbalMover:
    ...
    def callback(self, target_msg, gimb_orient_msg, target_GT, target_global_pos_msg, UAV_global_pos_msg):
        ...
        if wuav <= 1:
            ...
            [[xrot1],[yrot1],[zrot1]]=np.dot(UAV_rot_mat, np.array([[x],[y],[z]]))
        else:
            [[xrot1],[yrot1],[zrot1]]=[[x],[y],[z]]
        

        print("corrected relative position \nx= ",xrot1,"\ny= ",yrot1,"\nz= ",zrot1)

        xGP=xrot1 - corrected_position[0]
        yGP=yrot1 - corrected_position[1]
        
        #target_global_pos_msg.pose.x=xG
        #target_global_pos_msg.pose.y=yrot1
        #self.Target_global_pose_corrected.publish(target_global_pos_msg)
